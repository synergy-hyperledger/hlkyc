...
class StateList {
     ...
     */
    async addState(state,eventType) {
        let key = this.ctx.stub.createCompositeKey(this.name, state.getSplitKey());
        let data = State.serialize(state);
        await this.ctx.stub.putState(key, data);
        await this.ctx.stub.setEvent(eventType, data);
        console.log("inside addState : ",data);
    }
     ...
     */
    async updateState(state,eventType) {
        let key = this.ctx.stub.createCompositeKey(this.name, state.getSplitKey());
        let data = State.serialize(state);
        await this.ctx.stub.putState(key, data);
        await this.ctx.stub.setEvent(eventType, data);
    ...
    async setEvent(state,eventType){
        console.info("inside set event , event type ",eventType);
        let key = this.ctx.stub.createCompositeKey(this.name, state.getSplitKey());
        let data = State.serialize(state);
        await this.ctx.stub.setEvent(eventType,data);
    }
